...
class Outcome {
  ...
  static async getAllByStudentId(studentId) {
    const response = await db.query(
      "SELECT o.score, c.challenge_name, s.name FROM outcome as o JOIN student as s ON o.student_id = s.id JOIN challenge as c ON o.challenge_id = c.id WHERE s.id = $1;",
      [studentId],
    );
  ...
  static async getAverageScoreByStudentId(studentId) {
    const response = await db.query(
      "SELECT AVG(o.score) as average_score FROM outcome as o JOIN student as s ON o.student_id = s.id WHERE s.id = $1;",
      [studentId],
    );
    if (response.rows.length === 0) {
      throw new Error("Unable to locate results for this student.");
  ...
  static async create(data) {
    const { student_id, challenge_id, score } = data;

    if (!student_id || !challenge_id || score === undefined) {
      throw new Error("Missing required fields.");
    }
    const response = await db.query(
      `INSERT INTO outcome
            (student_id, challenge_id, score)
            VALUES ($1, $2, $3)
            RETURNING *;`,
      [student_id, challenge_id, score],
    );
